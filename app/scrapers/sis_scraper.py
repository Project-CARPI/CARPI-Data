...
async def get_courses(session, term, subject_code):
    ...
    async with session.get(url) as response:
        ...
        for class_link in subj_class_data:
            ...
            class_code_name = class_link.get_text().split(
                " - "
            ...
            task = asyncio.create_task(
                get_course_detail(session, term, subject_code, course_code)
            )
...
def get_section_crosslist(section_body):
    ...
    if "Cross List Courses:" in section_body:
        if "Prerequisites:" in section_body:
            crosslistBody = section_body[
                section_body.index("Cross List Courses:") : section_body.index(
                    "Prerequisites"
                )
            ]
        else:
...
def get_restrictions(section_body):
    ...
    section_body = (
        section_body.replace("&nbsp;", "").replace("\xa0", "").replace("   ", "")
    )
    while "\n\n" in section_body:
    ...
    if "Restrictions:" in section_body:
        if "Prerequisites:" in section_body:
            restriction_body = section_body[
                section_body.index("Restrictions:") : section_body.index(
                    "Prerequisites:"
                )
            ]
        else:
...
async def get_section_info(session, term, soup):
    ...
    for section_table in section_tables:
        ...
        for section_row in section_table.find_all("tr")[1:]:  # Skip the header row
            # per cell in row
            section_row_info = [
                section_cells.text for section_cells in section_row.find_all("td")
            ]
        ...
        section_crosslist, restrictions, capacity, registered, open_seats = (
            await get_section_data(session, term, CRN)
        )
        # Crosslist
...
async def fetch_crosslist_restriction_section(session, term, subject_code, course_code):
    ...
    async with session.get(url) as response:
        ...
        response_crosslist, response_restrictions, sections_data = (
            await get_section_info(session, term, soup)
        )
...
async def parse_attributes(soup, term, subject_code, course_code):
    attributes = []
    attribute_label = soup.find(
        "span", class_="fieldlabeltext", string="Course Attributes: "
    )
...
async def get_course_detail(session, term, subject_code, course_code):
    ...
    params = (
        f"cat_term_in={term}&subj_code_in={subject_code}&crse_numb_in={course_code}"
    )
    url = f"{url}?{params}"
    info = {
        "description": "",
        ...
        "crosslist": [],
        "attributes": [],
        "restrictions": {"major": [], "level": [], "classification": [], "grade": []},
        "credits": {"min": 0, "max": 0},
        "offered": "",
    }
    ...
    async with session.get(url) as response:
        ...
        course_data = (
            soup.find("td", class_="ntdefault").contents[0].strip().split("\n")
        )

        crosslist, restrictions, sections_data = (
            await fetch_crosslist_restriction_section(
                session, term, subject_code, course_code
            )
        )
        ...
        info["attributes"] = await parse_attributes(
            soup, term, subject_code, course_code
        )
        ...
        for semester in ["fall", "spring", "summer"]:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                for subject_code, subject_name in subjects.items():
                    task = asyncio.create_task(
                        get_courses(
                            session=session, term=term, subject_code=subject_code
                        )
                    )
                    tasks.append(task)
                ...
                for (subject_code, subject_name), courses in zip(
                    subject_metadata, courses_by_subject
                ):
